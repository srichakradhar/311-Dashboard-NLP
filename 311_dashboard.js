...
function UpdateDate(dateControl) {
	// console.log(dateControl.value);
	year = dateControl.value.split('-')[0];
}
...
function zoomToFeature(e) {
	...
	var categoriesChart = new CanvasJS.Chart("categories", {
		...
		data: [{
			...
			indexLabel: "{label} {y}",
			dataPoints: cat_deps['CATEGORIES'][year][zip_code]
		}]
	});
	categoriesChart.render();
	
	var departmentsChart = new CanvasJS.Chart("departments", {
		animationEnabled: true,
		title: {
			text: "Departments"
		},
		data: [{
			...
			yValueFormatString: "##0 \"Cases\"",
			indexLabel: "{label} {y}",
			dataPoints: cat_deps['DEPARTMENTS'][year][zip_code]
		}]
	});
...
/*
$.ajax({
    url: "https://data.kcmo.org/resource/7at3-sxhp.json",
    type: "GET",
    data: {
      "$limit" : 2,
      "$$app_token" : "PiDJSsyICjOdXdu9CRQWTRkyP"
    }
}).done(function(data) {
  alert("Retrieved " + data.length + " records from the dataset!");
  console.log(data);
});
// Referece: https://dev.socrata.com/foundry/data.kcmo.org/7at3-sxhp
*/